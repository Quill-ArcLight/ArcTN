...
MPI（Message Passing Interface，消息传递接口）让多个进程、也可以是多台机器协同计算。
ArcTN 通过可选的 `mpi` feature 提供切片并行执行。普通单机 Rust/Python 功能不需要 MPI；
`mt` 是单机矩阵乘法多线程，与 MPI 是两个独立选项。

多进程测试覆盖 `f32`、`f64`、`complex64`、`complex128` 四种数据类型，以及
1、2、4 个 MPI 进程的计算结果。测试同时检查无切片执行、进程数多于切片数的情况和无效输入。
...
可选依赖链 `mpi → conv → custom_derive 0.1.7` 有停止维护公告 `RUSTSEC-2025-0058`。
本次保留锁定版本，由 liuxl 在 2026-10-15 前复查；接受范围仅限该维护公告。
新的漏洞、其他公告、版本或包校验和变化，以及到期都会使发布审查失败。
